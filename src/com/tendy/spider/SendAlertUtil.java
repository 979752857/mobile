...
public class SendAlertUtil {
    ...
    public static void init(){
        cityMap = new HashMap<>();
        List<PhoneAlertConfig> configList = DataMapperUtil.selectPhoneAlertConfigAllConfig();
        if(CollectionUtils.isEmpty(configList)){
            return;
        }
        for(PhoneAlertConfig config : configList){
            Map<String, String> item = new HashMap<>();
            item.put("cityId", String.valueOf(config.getCityId()));
            item.put("tag", config.getTag());
            item.put("key", config.getContainsKey());
            item.put("send", config.getSendAlert());
            List<Map<String, String>> list = cityMap.get(config.getCityId());
            if(list == null){
                list = new ArrayList<>();
            }
            list.add(item);
            cityMap.put(config.getCityId(), list);
        }
    }
