...
public class IndexService {
	...
	public ReplyMap getData(String phoneParam, String businessCid, Integer pageNo, Integer pageSize, String status, String tag, String notPhone, String position, String type) {
		...
		//所有城市运营商能看到开放城市的账号号码
		if (Constants.CONFIG_TRUE_NUMBER.equals(roleMap.get(Constants.OPEN_BUSINESS_KEY) != null
				? String.valueOf(roleMap.get(Constants.OPEN_BUSINESS_KEY)) : "")) {
		...
		//没有任何条件参数 走默认查询
		if (StringUtils.isBlank(phoneParam) && StringUtils.isBlank(tag)
				&& StringUtils.isBlank(notPhone) && StringUtils.isBlank(type)
		...
		boolean isGeomantic = Constants.CONFIG_TRUE_NUMBER.equals(roleMap.get(Constants.GEOMANTIC_KEY) != null ? String.valueOf(roleMap.get(Constants.GEOMANTIC_KEY)) : "");
